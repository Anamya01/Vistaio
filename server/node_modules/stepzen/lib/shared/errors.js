...
const formatTemporaryErrorMessage = (errors) => `A problem occurred while processing your command${errors.length > 0 && errors[0].message ? `: "${errors[0].message}"` : ''}. Please try again.` +
...
exports.PERMANENT_STEPZEN_ERROR = 'An unexpected problem occurred while processing your command.' +
    ` If the issue persists, make sure you are on the latest version of StepZen ` +
    `CLI. And if the issue still persists contact support via the StepZen discord ` +
    `channel (${constants_1.STEPZEN_DISCORD_URL}).`;
