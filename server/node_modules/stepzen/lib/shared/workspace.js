...
const initWorkspace = async (args) => {
    ...
    if (args.nonInteractive && questions.some(question => question.when)) {
        throw new errors_1.CLIError(chalk `Missing parameters for creating a StepZen workspace in {bold --non-interactive} mode` +
            chalk ` (see {bold stepzen init --help} for more information)`);
    }
    // Get the answers
    const answers = {
        name,
        endpoint,
        'use-root': true,
        ...(await inquirer.prompt('workspace', questions)),
    };
