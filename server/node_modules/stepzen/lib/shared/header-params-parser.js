...
const parseHeaderParam = (headers, headerParam) => {
    const { result: header, error: headerError } = (0, header_1.parseHeader)(headerParam);
    if (headerError || !header) {
        ...
        return {
            error: {
                message: `Could not find a header name in` +
    ...
    if (!paramValueMatches || !paramValueMatches.groups) {
        return {
            error: {
                message: `Could not find a $-prefixed parameter name in` +
                    ` ${chalk.bold(`--header-param '${headerParam}'`)}.` +
    ...
    if (suffix.match(headerParamRegex)) {
        return {
            error: {
                message: `StepZen CLI currently supports at max 1 parameter per header` +
    ...
    if (candidates.length === 0) {
        return {
            error: {
                message: `Could not find a matching '-H, --header' curl flag for` +
                    ` ${chalk.bold(`--header-param '${headerParam}'`)}.` +
...
const makeHeaders = (curlHeaders, headerParams = []) => {
    ...
    for (const headerParam of headerParams) {
        const { result, error } = (0, exports.parseHeaderParam)(remainingCurlHeaders, headerParam);
        if (error) {
            return { error };
        }
        headers.push(result.header);
        remainingCurlHeaders = result.remainingHeaders;
    }
